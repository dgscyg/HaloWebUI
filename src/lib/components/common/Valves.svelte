<script>
	...
	function isSensitiveField(property) {
		return SENSITIVE_PATTERN.test(property);
	}
</script>
				...
				<div class="flex mt-0.5 mb-1.5 space-x-2">
					<div class=" flex-1">
						...
						{:else if (valvesSpec.properties[property]?.type ?? null) !== 'string'}
							...
							/>
						{:else if isSensitiveField(property) && (valvesSpec.properties[property]?.type ?? 'string') === 'string'}
							<SensitiveInput
								placeholder={valvesSpec.properties[property].title}
								bind:value={valves[property]}
								on:change={() => {
