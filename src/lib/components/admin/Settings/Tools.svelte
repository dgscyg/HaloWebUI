<script lang="ts">
	...
	> = {
		native: {
			label: '内置功能',
			description: '管理工具调用模式、内置搜索、知识库、图像生成等原生工具开关。',
		...
		workspace: {
			label: '工作空间工具',
			description: '管理自定义 Python 工具，支持导入、导出和阀门配置。',
			badgeColor: 'bg-blue-50 dark:bg-blue-950/30',
			iconColor: 'text-blue-500 dark:text-blue-400'
		},
		openapi: {
			label: 'OpenAPI 服务器',
			description: '连接兼容 OpenAPI 规范的工具服务器，适用于企业级集成。',
	...
	const normalizeMCPAppsCapabilities = (value: Record<string, any> | null | undefined) => ({
		...
		servers: Array.isArray(value?.servers)
			? value.servers.map((server: any) => ({
					...
					server: server?.server && typeof server.server === 'object' ? server.server : {},
					capabilities:
					...
					metadata: server?.metadata && typeof server.metadata === 'object' ? server.metadata : {}
				}))
			: []
	});
	...
	const resetSection = (section: 'native' | 'mcp' | 'openapi') => {
		...
		if (section === 'native') {
			...
			mcpServers = cloneSettingsSnapshot(initialSnapshot.mcp.servers).map(normalizeMCPServer);
					...
					>
						...
						>
							{#if canManageGlobalToolPolicies}
								<button
									type="button"
									class={`flex min-w-0 items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition-all ${selectedTab === 'native' ? 'bg-white text-gray-900 shadow-sm dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200'}`}
									on:click={() => {
										selectedTab = 'native';
									}}
								>
							...
							<button
								type="button"
								class={`flex min-w-0 items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition-all ${selectedTab === 'mcp' ? 'bg-white text-gray-900 shadow-sm dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200'}`}
								on:click={() => {
									selectedTab = 'mcp';
								}}
							>
							...
							<button
								type="button"
								class={`flex min-w-0 items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition-all ${selectedTab === 'workspace' ? 'bg-white text-gray-900 shadow-sm dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200'}`}
								on:click={() => {
							...
							<button
								type="button"
								class={`flex min-w-0 items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition-all ${selectedTab === 'openapi' ? 'bg-white text-gray-900 shadow-sm dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200'}`}
								on:click={() => {
									selectedTab = 'openapi';
								}}
							>
			...
			{:else if selectedTab === 'mcp'}
				...
				>
					...
					<div class="glass-item p-4">
						...
						<div
							class="mt-4 flex items-center justify-between gap-3 rounded-xl border border-gray-200/70 bg-white/70 p-3 dark:border-gray-800/70 dark:bg-gray-900/40"
						>
							<div>
						...
						{#if mcpServers.length > 0}
							<div class="mt-4 space-y-3">
								{#each mcpServers as server, idx}
									...
									<div
										class="rounded-xl border border-gray-200/70 bg-white/70 p-3 dark:border-gray-800/70 dark:bg-gray-900/40"
									>
										<div class="flex items-start justify-between gap-3">
											<div class="min-w-0 flex-1">
												<div class="flex flex-wrap items-center gap-2">
													...
													<span
														class="rounded-full px-2 py-0.5 text-[10px] font-medium {server.config
															?.enable
															? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300'
															: 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}"
													>
														{server.config?.enable
															? $i18n.t('服务器已启用')
															: $i18n.t('服务器已禁用')}
													</span>
													<span
														class="rounded-full px-2 py-0.5 text-[10px] font-medium {capability?.apps_enabled
															? 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300'
															: 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}"
													>
														{capability?.apps_enabled
															? $i18n.t('Apps 已生效')
															: $i18n.t('Apps 未生效')}
													</span>
												</div>
										...
										<div class:hidden={!resourcePreview.length} class="mt-3">
											<div
												class="mb-1 text-[11px] font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400"
											>
												{$i18n.t('可渲染资源')}
											</div>
											<div class="space-y-1.5">
												{#each resourcePreview as resource}
													<div
														class="rounded-lg border border-gray-200/70 px-3 py-2 text-xs dark:border-gray-800/70"
													>
						...
						{:else}
							<div
								class="mt-3 rounded-xl border border-dashed border-gray-200/80 px-4 py-4 text-sm text-gray-500 dark:border-gray-800/80 dark:text-gray-400"
							>
