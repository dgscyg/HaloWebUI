    ...
    for node_id in history["outputs"]:
        ...
        if "images" in node_output:
            for image in node_output["images"]:
                # Only include final output images, skip temporary previews
...
def _upload_image_to_comfyui(image_data: bytes, base_url: str, api_key: str) -> str:
    """Upload an image to ComfyUI's /upload/image endpoint and return the filename."""
    import io
    import uuid
...
async def comfyui_generate_image(
    ...
    # If img2img: upload source image to ComfyUI first
    uploaded_image_name = None
    if payload.image_b64:
        import base64

        image_data = base64.b64decode(payload.image_b64)
        uploaded_image_name = await asyncio.to_thread(
            _upload_image_to_comfyui, image_data, base_url, api_key
        )
    ...
    for node in payload.workflow.nodes:
        if node.type:
            ...
            elif node.type == "image" and uploaded_image_name:
                for node_id in node.node_ids:
                    workflow[node_id]["inputs"][
                        node.key if node.key else "image"
                    ] = uploaded_image_name
            elif node.type == "denoise":
                denoise_val = payload.denoise if payload.denoise is not None else 0.75
                for node_id in node.node_ids:
                    workflow[node_id]["inputs"][
                        node.key if node.key else "denoise"
                    ] = denoise_val
        else:
            for node_id in node.node_ids:
