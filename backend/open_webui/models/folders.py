...
class FolderTable:
    ...
    def update_folder_icon_by_id_and_user_id(
        self, id: str, user_id: str, icon: Optional[str]
    ) -> Optional[FolderModel]:
        try:
            with get_db() as db:
                folder = db.query(Folder).filter_by(id=id, user_id=user_id).first()
            ...
            log.error(f"update_folder_icon: {e}")
            return

    def update_folder_system_prompt_by_id_and_user_id(
        self, id: str, user_id: str, system_prompt: Optional[str]
    ) -> Optional[FolderModel]:
        try:
            with get_db() as db:
                folder = db.query(Folder).filter_by(id=id, user_id=user_id).first()
            ...
            log.error(f"update_folder_system_prompt: {e}")
            return
    ...
    def delete_folders_by_user_id(self, user_id: str) -> bool:
        try:
            ...
            log.error(f"delete_folders_by_user_id: {e}")
            return False
