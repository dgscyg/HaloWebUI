...
class ModelsTable:
    ...
    def bulk_upsert_base_models(
        ...
        This is used by admin UX to manage huge model lists efficiently without
        requiring per-model editor navigation.

        Supported patch fields:
        - is_active: bool
        - access_control: dict | None (None => public)
        - meta: dict (shallow-merged into existing meta)
        """
        if not items:
        ...
        if not ids:
            return {"updated": 0, "created": 0, "skipped": len(items)}
        ...
        try:
            with get_db() as db:
                ...
                for item in items:
                    ...
                    # Only operate on base model overrides
                    ...
                    if "is_active" in patch and isinstance(patch_is_active, bool):
                        row.is_active = patch_is_active
                        changed = True

                    if "access_control" in patch:
                        # Can be None (public) or a dict (private / restricted)
                        row.access_control = patch_access_control
                        changed = True

                    if "meta" in patch and isinstance(patch_meta, dict):
                        # Shallow-merge meta to avoid clobbering other keys (e.g. profile_image_url).
                        existing_meta = row.meta or {}
                        if not isinstance(existing_meta, dict):
                            existing_meta = {}
                        row.meta = {**existing_meta, **patch_meta}
                        changed = True
