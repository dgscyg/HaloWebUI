...
class ToolsTable:
    ...
    def get_tools(self) -> list[ToolUserModel]:
        with get_db() as db:
            ...
            user_ids = list({t.user_id for t in all_tools})
            users_map = Users.get_users_map_by_ids(user_ids) if user_ids else {}
            tools = []
            for tool in all_tools:
                user = users_map.get(tool.user_id)
                tools.append(
    ...
    def get_tools_list(self) -> list[ToolUserResponse]:
        """Lightweight listing: excludes heavy content/specs columns, batch user lookup."""
        with get_db() as db:
            all_tools = (
                db.query(
                    Tool.id,
            ...
            for t in all_tools:
                ...
                results.append(
                    ToolUserResponse(
                        ...
                        user_id=t.user_id,
                        name=t.name,
                        meta=ToolMeta(**(t.meta if isinstance(t.meta, dict) else {})),
