...
| 自定义请求头 | `http` 时可选  | 额外 HTTP 请求头，保留头会被拒绝               |
| 密钥         | 条件           | Bearer / OAuth 认证所需的令牌                  |
| 启用         | 是             | 是否启用该服务器                               |

`stdio` 连接仅允许管理员保存和验证。

界面会额外返回当前运行时能力信息，用于提示 `npx` / `uvx` / `git` 等命令在当前镜像内是否可直接使用。
...
- `MCP Apps` 的生效状态同时受全局开关、单服务器开关、以及服务器自身启用状态影响。
- 保存服务器配置与保存 `MCP Apps` 配置是两条独立持久化链路。
...
**Q: HTTP MCP 为什么会回退到 legacy SSE?**

部分旧版服务端不支持新的 Streamable HTTP 交互方式。当前实现会在特定状态码下自动尝试兼容旧版 SSE 流，无需额外开关。
