...
- `backend/open_webui/models/groups.py` (GroupTable.get_group_by_name): 通过归一化名称直接查询用户组，供游客组分配复用。
...
1. **入口检查**: `backend/open_webui/routers/auths.py` (`guest_session`) 先检查 `ENABLE_GUEST_ACCESS`
2. **用户创建**: 使用随机 `guest-<hex>@guest.local` 邮箱创建本地账号，角色被收敛为 `user`
3. **组分配**: `_ensure_user_in_group()` 确保 `Guest` 组存在并把新游客加入其中
4. **Token 签发**: `_issue_session_response()` 签发带 `guest=true` 的 JWT 并写入认证 Cookie
...
游客会话使用 `backend/open_webui/routers/auths.py` 中的 `_guest_group_lock` 串行化首个 `Guest` 组创建流程；组解析通过 `backend/open_webui/models/groups.py` (`get_group_by_name`) 直接查询数据库，避免为单次游客登录扫描全部用户组。
