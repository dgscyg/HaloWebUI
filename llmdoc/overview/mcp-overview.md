...
HaloWebUI 支持通过 MCP 协议连接外部工具服务器，当前同时支持 Streamable HTTP 与本地 `stdio` 两种传输方式。系统集成后，AI 模型可自动发现并调用 MCP 服务器提供的工具，并可在启用 `MCP Apps` 时读取服务器暴露的资源与提示能力。

### 2.1 协议规范

| 属性 | 值 |
|------|-----|
| 协议版本 | 默认优先 `2025-06-18`，可协商回退至旧版 |
| 传输方式 | Streamable HTTP、`stdio` |
| 通信协议 | JSON-RPC 2.0 |
| 响应格式 | HTTP: JSON / SSE；`stdio`: JSON Lines |
...
- **资源与提示发现**: 可读取 `resources/list`、`resources/read` 与 `prompts/list`
- **MCP Apps**: 支持代理 `ui://` 资源到前端渲染，并保留每服务器启用开关
- **会话管理**: HTTP 连接支持 MCP Session ID 自动管理
- **协议协商**: HTTP/`stdio` 初始化阶段均支持协议版本协商回退
...
| Zapier | Zapier 自动化集成 |
| Memory / Sequential Thinking / Context7 / Fetch / Time | 本地 `stdio` MCP 服务器模板 |
