...
完整实现 MCP (Model Context Protocol) 的 HTTP + `stdio` 双传输集成:

- **协议版本**: 默认优先 `2025-06-18`，初始化阶段可协商回退旧版
- **传输方式**: Streamable HTTP、本地 `stdio`
- **认证方式**: None、Bearer、Session、OAuth 2.1
- **扩展能力**: 支持 `MCP Apps`、资源读取、提示发现、legacy SSE fallback
- **预设服务器**: Composio、Smithery、Zapier，以及 Memory / Sequential Thinking / Context7 / Fetch / Time 等本地 `stdio` 模板
